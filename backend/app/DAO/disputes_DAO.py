...
class DisputesDAO:
    ...
    # Automatically assigns dispute id
        ...
        # Replace None with empty string for specific string fields
        for field in ["reason", "solution", "description"]:
            if dispute_data[field] is None:
                dispute_data[field] = ""
    ...
    async def update_dispute(self, updateDetails: Disputes) -> None:
        ...
        await self.collection.update_one(
            {"ticket_number": updateDetails.ticket_number},
            {"$set": update_data}
        )
